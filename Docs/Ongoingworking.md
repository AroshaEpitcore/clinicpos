...
**Currently working on:** Phase 5.6 polish (dual-queue, i18n, caps, portal hours all shipped 2026-05-20)
**Last updated:** 2026-05-20
**Next up:** Phase 6 — payment gateway integration, SMS reminders, WhatsApp integration

### 2026-05-20 — Phase 5.6 features shipped
- **Dual Token Queue** — red `N-XX` for new patients, blue `XX` for returning. Two-level gate (super-admin capability + clinic-admin opt-in). Auto-detect from consultation history with audit-logged override. POS + online booking + queue display + thermal slip all visit-type aware.
- **Sinhala Patient Portal** — EN ⇄ සිං toggle on all patient pages (`/book` + `/patient/*`). Choice persists in localStorage. In-house i18n module (`src/i18n/`).
- **Daily Caps** — clinic-wide (`clinic_settings.max_patients_per_day`) AND per-doctor (`staff.max_patients_per_day`). Enforced in POS and online booking. Emergencies bypass.
- **Online Booking Hours** — per-day-of-week schedule (`portal_hours` table). Closed-state UI on `/book` with next-open time; `POST /portal/book` returns 403 outside hours.
- **Tests** — first Jest + Supertest suite in `backend-api/__tests__/`. 22 passing.
- **Migrations** — `migrate_dual_queue.js`, `migrate_dual_queue_clinic_toggle.js`, `migrate_caps_and_portal_hours.js` (all idempotent).
- **Deploy doc** — `Docs/HOSTING_SESSION.md` updated with smoke tests.
...
| **Patient Login Portal (Phase 5.5 — Auth)** | ✅ Patients log into their own health portal at `/patient/*`. `portal_password_hash` + `portal_registered_at` columns added via `migrate_patient_portal_auth.js`. Endpoints: POST /register, POST /login, GET /me, GET /summary, GET /appointments, GET /consultations, GET /prescriptions, GET /labs, GET /invoices, GET /invoices/:id, PUT /profile, PUT /change-password. `PatientAuthContext` stores patient JWT (key: `patient_token`) separately from staff JWT. 10 portal pages: Dashboard (summary cards + quick links), Appointments, Consultations, Prescriptions, Labs, Invoices + detail modal, Profile, Login, Register. `patientAuthMiddleware` verifies patient JWT + sets `req.patient`. `patient_login_enabled` flag in `clinic_settings` gates all portal access. (2026-04-28) |
| **Patient Portal — Design System Compliance + Dark Mode + Validation** | ✅ All 10 portal pages bulk-fixed: undefined CSS vars (`--color-ink`, `--color-ink-light`, `--color-ink-faint`) replaced with `--color-text`/`--color-text-secondary`; `rounded-2xl` → `rounded-[var(--radius-lg)]`; `border-2` → `border`; `py-3.5` → `py-2`; `bg-white` → `bg-[var(--color-surface)]`; all `style={{}}` inline styles removed. `PatientLayout` updated with `useTheme()` dark mode toggle (Sun/Moon icon in header). `PatientLoginPage` and `PatientRegisterPage` fully rewritten as clean centered card — matches clinic `LoginPage.jsx` pattern. `formatPhoneInput()` + `validatePhone()` from `utils/format.js` on all phone inputs. Field-level validation: red `border-[var(--color-danger)]` + inline error message below each field, errors clear on input change. `PatientProfilePage` rebuilt with same validation pattern. All data pages: `.catch(() => {})` silent swallows replaced with `toast.error()`. (2026-05-01) |
| **Patient Portal — Backend Column Name Fixes** | ✅ All portal data pages showed empty lists despite data existing in admin view. Root causes: `prescription_items` column is `instructions` not `food_instruction` — fixed with `pi.instructions AS food_instruction`; `invoice_items` column is `total_price` not `line_total` — fixed with `total_price AS line_total`; `payment_splits` column is `recorded_at` not `created_at` — fixed with `recorded_at AS created_at`; invoices list was selecting `i.status` which doesn't exist — correct column is `i.payment_status AS status`. All four fixes in `patient-portal.routes.js`. Errors were previously hidden by silent `.catch(() => {})` on all frontend pages. (2026-05-01) |
| **`api/index.js` Production BaseURL Fix** | ✅ `VITE_API_URL` is intentionally empty string in production (API is proxied via Nginx). Empty string is falsy — fell back to hardcoded `http://localhost:4000/api/v1`. All API calls from the patient's browser in production went to their own local machine (visible in network tab as 400 localhost:4000 requests). Fixed: `window.location.protocol + '//' + window.location.host + '/api/v1'` as dynamic fallback — resolves correctly on any subdomain (demo.healthcenter.lk, familycare.healthcenter.lk, etc.) and still works in local dev when `VITE_API_URL` is set. (2026-05-01) |
...
| 2026-04-24 | Landing page full light theme conversion (all dark sections → light), navbar hash-link scroll fix, mobile drawer solid background fix. Bug fixes: logo caching (timestamp filename), AuthContext currency field. Nginx conflicting server_name fixed. PM2 auto-start via systemd. Full docs update (BUGS.md, INSTRUCTION.md, RUNNING.md, workflow.md, Ongoingworking.md) | Complete | — |
| 2026-04-25 | Navbar mobile menu rewrite — left-side slide drawer with backdrop, body scroll lock, active link highlight, pinned CTA. Booking QR Code Card feature — Settings → Security tab shows live QR preview + "Download QR Card (PNG)" button (branded canvas card). `qrcode.react` installed in clinic-frontend. | Complete | — |
