...
- **InvoiceModal** — view and manage the invoice:
  ...
  - **Record Payment** — select method (Cash / Card / Online / Insurance / **QR Pay**), enter amount, optional reference field (not shown for QR Pay)
  - **QR Pay flow:** selecting "QR Pay" and clicking Pay opens a full-screen QR dialog. Dialog shows the clinic's Lanka QR image (uploaded by admin), the exact invoice amount, a bank-app instruction line, and a "Download QR" link. "Payment Received" button in the dialog completes the invoice as paid. "Cancel" returns to the invoice without recording payment.
...
- Right panel: per-method breakdown (Cash / Card / Online / Insurance / **QR Pay**) + cash count input
...
- Past closed days show read-only summary with who closed it and when; Payment Breakdown includes QR Pay total
...
- **Settings page** (`/settings`) — 9 tabs:
  ...
  - **Billing** — currency code, tax label, tax rate (%). **QR Payment** — upload clinic's Lanka QR bank image (JPG/PNG ≤2MB) with 160×160 preview and delete button. This image is shown to patients during QR Pay checkout.
  ...
  - **Website** — enable/disable public clinic website; hero image (URL or device upload ≤5MB); tagline; about text; clinic hours; Google Maps embed URL; WhatsApp number; Facebook URL
...
| **Public Clinic Website** | ✅ Complete | `PublicClinicPage.jsx` at `/` (public, no auth). Hero with CSS keyframe animations + stats bar + trust badges + ambient glow orbs; About + WHY_US cards; Doctors dark-theme cards; Services with keyword icon mapper (`svcIcon()`); Contact colour-coded cards; CTA 2-col; Footer 4-col. `RevealBlock` scroll-reveal via IntersectionObserver. Responsive mobile layout + animated hamburger navbar. Settings → Website tab (9th tab): enable toggle, tagline, about, hours, map URL, WhatsApp, Facebook, hero image (URL + device upload). Hero image upload saves to `/uploads/.../hero/` (5MB limit). `migrate_website_settings.js` adds 8 website fields to all tenant `clinic_settings`. (2026-04-26) |
| **Lanka QR Payment** | ✅ Complete | Static QR payment at counter using Lanka QR standard. `migrate_qr_payment.js` adds `qr_image_url`/`qr_image_filename` to `clinic_settings` and `qr_total DECIMAL(10,2)` to `end_of_day`. `POST/DELETE /settings/qr-image` endpoints (multer, 2MB, images only). Settings → Billing tab: QR upload card with preview. InvoiceModal: 5th payment method "QR Pay" — shows full-screen QR dialog with image, amount, bank app instruction, Download QR link; "Payment Received" completes the invoice. EOD: `qr_total` in live summary and closed-day breakdown. Help page updated. Landing page: `QrPaymentSection.jsx` + FeaturesSection item #17. (2026-04-30) |
...
| Help & User Guide page | ✅ Done (2026-04-22) — /help page all roles + /guide React route on landing-frontend (GuidePage.jsx) |
...
| Public clinic website per-tenant | ✅ Done (2026-04-26) — `PublicClinicPage.jsx` at `/`. Full redesign with CSS animations, stats bar, scroll-reveal sections. Settings → Website tab (9th tab). Hero image upload from device. `migrate_website_settings.js` adds 8 website fields. |
| QR payment (Lanka QR static at counter) | ✅ Done (2026-04-30) — Settings upload, InvoiceModal QR Pay method + dialog + download, EOD qr_total |
