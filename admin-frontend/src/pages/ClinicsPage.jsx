...
const STATUS_TABS = ['all', 'active', 'trial', 'suspended'];

function CreateClinicModal({ open, onClose, onCreated }) {
  const [form, setForm] = useState({
    clinic_name: '', subdomain: '', owner_email: '', owner_phone: '',
    plan: 'basic', trial_days: 30,
  });
  ...
  // Auto-generate subdomain from clinic_name
  function onNameBlur() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      onClose();
      setForm({ clinic_name: '', subdomain: '', owner_email: '', owner_phone: '', plan: 'basic', trial_days: 30 });
    } catch (err) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Create New Clinic" size="md">
  ...
  return (
    <div className="space-y-5">
              ...
              <tbody className="divide-y divide-gray-100">
                {clinics.map(c => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5"><Badge label={c.plan} variant={c.plan} /></td>
      ...
      <CreateClinicModal
        ...
        onCreated={newClinic => setClinics(prev => [newClinic, ...prev])}
      />
    </div>
  );
}
