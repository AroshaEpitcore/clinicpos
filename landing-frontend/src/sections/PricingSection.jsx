...
export default function PricingSection() {
  ...
  return (
    <section id="pricing" className="py-16 md:py-20 lg:py-24 bg-ink text-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {!loading && !error && plans.some(p => Number(p.yearly_price) > 0) && (
          <FadeIn className="flex items-center justify-center gap-3 mb-10 md:mb-12">
            <span className={`text-sm font-medium ${!yearly ? 'text-white' : 'text-white/60'}`}>Monthly</span>
            <label className="relative w-12 h-[26px] cursor-pointer block">
              ...
              <span onClick={() => setYearly(v => !v)} className="absolute inset-0 rounded-full cursor-pointer bg-gradient-to-br from-indigo-500 to-indigo-600">
            ...
            <span className={`text-sm font-medium ${yearly ? 'text-white' : 'text-white/60'}`}>Yearly</span>
            <span className="text-[0.72rem] font-bold px-2.5 py-[3px] rounded-full border bg-emerald-500/[0.15] text-green-400 border-emerald-500/25">
              Save 2 months
            </span>
          </FadeIn>
        )}
        ...
        {error && !loading && (
          <p className="text-center py-10 text-sm text-white/60">
            Pricing information coming soon.{' '}
            <a href="#contact" className="text-indigo-300 hover:text-indigo-200 transition-colors">Contact us</a> for details.
          </p>
        )}
            ...
            {plans.map((plan, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-base font-bold mb-1.5">{plan.name}</div>
                  <div className="text-sm mb-6 min-h-9 text-white/60">
                    {plan.description || 'Great for growing clinics'}
                  </div>
                  <div className="mb-7">
                    {price > 0 ? (
                      <>
                        <span className="text-[2.8rem] font-black tracking-[-2px] leading-none">
                          LKR {price.toLocaleString()}
                        </span>
                        <span className="text-sm ml-1 text-white/60">{period}</span>
                        {isYearly && (
                          <div className="text-[0.75rem] mt-1.5 text-indigo-300">Billed annually</div>
                        )}
                      </>
                    ) : (
                      <span className="text-[1.6rem] font-black">Contact Us</span>
                    )}
                  </div>
                  <a href="#contact"
                  ...
                  <ul className="flex flex-col gap-2.5">
                    {(plan.features || ['Unlimited patients','All core modules','Free onboarding','Email support']).map((f, fi) => (
                      <li key={fi} className="flex items-start gap-2.5 text-sm text-white/60">
                        <span className="w-[18px] h-[18px] bg-emerald-500/20 text-green-400 rounded-[5px] shrink-0 flex items-center justify-center text-[0.65rem] mt-[1px]">✓</span>
                        {f}
                      </li>
                    ))}
                  </ul>
